...
// Safe AUTH import — dashboard never calls AUTH.calcLevel or AUTH.canStartSession
// Those methods don't exist on the AUTH object; all XP/level logic lives here.
let AUTH = {};
try {
  const mod = await import('./auth.js');
  AUTH = mod.AUTH || mod.default || {};
} catch (e) {
  console.warn('[dashboard] auth.js not loaded:', e.message);
}
...
async function renderLeaderboard() {
  ...
  let user = {};
  try { if (typeof AUTH.currentUser === 'function') user = AUTH.currentUser() || {}; } catch(e) {}
...
/* Safe logout — works regardless of AUTH shape */
...
async function bootDashboard() {
  // 1. Auth guard — safely read current user regardless of AUTH shape
  let user = {};
  try {
    if (typeof AUTH.currentUser === 'function') {
      user = AUTH.currentUser() || {};
    } else if (AUTH.currentUser && typeof AUTH.currentUser === 'object') {
      user = AUTH.currentUser;
    }
  } catch (e) {
    console.warn('[dashboard] Could not read currentUser:', e.message);
  }
