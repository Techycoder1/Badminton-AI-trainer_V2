...
window.addEventListener('load', async () => {
  ...
  if (profAvatar) profAvatar.textContent = (user.username || '?')[0].toUpperCase()
  if (profName)   profName.textContent   = user.username || '—'
  if (profEmail)  profEmail.textContent  = user.email    || '—'
  if (profRole)   profRole.textContent   = (user.role || 'student').charAt(0).toUpperCase() + user.role?.slice(1)
  if (setUser)    setUser.textContent    = user.username || '—'
